...
// ===== 🚀 修复后的 AI 定位器 =====
let aiBestCard = null;    // AI 推荐的卡牌 ID
let aiBestMode = null;    // AI 推荐的行动区 (top/bottom)
...
async function fetchAIRecommendation() {
  ...
  if (!coachOn || legal.length === 0) {
    aiBestCard = null; aiBestMode = null;
    render(); 
    return;
  }
...
function render(){
  ...
  if(uiMode==="normal"){
    if(isAiThinking) {
      actionArea.innerHTML = "<p style='color: #888; font-weight: bold; font-style: italic;'>🧠 机械神明 V5 正在推演局势...</p>";
      return; 
    }

    legal.forEach((a, i) => {
      // 🚀 核心：用卡牌 ID 和行动模式双重校验，绝不抓错！
      const isBest = (a.card_id === aiBestCard && a.mode === aiBestMode);
      ...
      b.textContent = `${i+1}. ${cardById(a.card_id).name} - ${a.mode==="top"?"取资源":a.kind} ${isBest?'(✨ V5 神明推荐)':''}`;
